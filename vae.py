...
class VAE(nn.Module):
    ...
    def decoder(self, z):
        ...
        logvar = torch.tanh(self.linear42(h_relu))
    ...
    def elbo(self, x):
        ...
        log_px_ifz = torch.sum(
            ...
            + (-0.5 * logvar)
            + ((-0.5 / torch.exp(logvar)) * (x - theta) ** 2.0),
            dim=1,
        )

        return log_px_ifz, kl
    ...
    def sample(self, num_samples):
        ...
        Returns:
            samples: Samples generated by the model, shape [num_samples, obs_dim]
        """
        ##########################################################
        # YOUR CODE HERE
        zp = torch.normal(0, 1, size=(num_samples, self.latent_dim))
        theta, logvar = self.decoder(zp)
...
x_sample = vae.sample(1000).detach().cpu().numpy()
